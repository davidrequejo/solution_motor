...
if (!isset($_SESSION["user_nombre"])) {
  ...
  if ($_SESSION['gasto_trabajador'] == 1) {
    ...
    switch ($_GET["op"]){
      ...
      case 'guardar_editar':
        ...
        if ( empty($idgasto_de_trabajador) ) { #Creamos el registro
          ...
          $rspta = $gasto_trab->insertar($idtrabajador, $tipo_gasto_modulo, $idcategoria, $tipo_comprobante, $serie_comprobante, 
          ...
          $rspta = $gasto_trab->editar($idgasto_de_trabajador, $idtrabajador, $tipo_gasto_modulo, $idcaja, $idcategoria, $tipo_comprobante, $serie_comprobante, 
          $fecha, $mes, $idproveedor, $precio_sin_igv, $igv, $val_igv, $precio_con_igv, $descr_comprobante, $img_comprob);
          echo json_encode($rspta, true);
        }
          ...
          while ($reg = $rspta['data']->fetch_object()) {
            ...
            $data[] = [
              ...
              "1" =>  '<div class="hstack gap-2 fs-15">' .
                '<button class="btn btn-icon btn-sm btn-warning-light" onclick="mostrar_editar_gdt('.($reg->idingreso_egreso_interno).')" data-bs-toggle="tooltip" title="Editar"><i class="ri-edit-line"></i></button>'.
                '<button  class="btn btn-icon btn-sm btn-danger-light product-btn" onclick="eliminar_gasto('.$reg->idingreso_egreso_interno.', \''.$reg->trabajador.'\')" data-bs-toggle="tooltip" title="Eliminar"><i class="ri-delete-bin-line"></i></button>'.
                '<button class="btn btn-icon btn-sm btn-info-light" onclick="mostrar_detalles_gasto('.($reg->idingreso_egreso_interno).')" data-bs-toggle="tooltip" title="Ver"><i class="ri-eye-line"></i></button>'.
              '</div>',
              ...
              "3" => '<div class="d-flex flex-fill align-items-center">
                ...
                <div>
                  <span class="d-block fw-semibold text-primary text-nowrap">'.$reg->trabajador.'</span>
                  <span class="text-muted">'.$reg->tipo_documento_nombre .' '. $reg->numero_documento.'</span>
                </div>
              </div>',
              "4" => $reg->tipo_comprobante .'<br>'. $reg->serie_comprobante,
      ...
      case 'seleccionar_ie_categoria':
        $rspta = $gasto_trab->seleccionar_ie_categoria();
        $data = "";
