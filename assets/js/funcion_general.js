...
function doc_view_extencion(filename, url_carpeta='',  width='50%', height='auto', error_img = false,  url_complete = false) {
  ...
  if (url_complete == true) {
    ...
    ruta =  `${window.location.origin}/libreria_sistema/${url_carpeta}/${filename}`;     
    host = window.location.host == 'localhost' || es_numero(parseFloat(window.location.host)) == true ? `${window.location.origin}/libreria_sistema/${url_carpeta}/${filename}` : `${window.location.origin}/${url_carpeta}/${filename}` ;    
  }  
...
function doc_view_download_expand(filename, ruta='', nombre_decarga='', width='50%', height='auto', error_img = false) {
  ...
  var ruta_file = window.location.host == 'localhost' || es_numero(parseFloat(window.location.host)) == true ? `${window.location.origin}/libreria_sistema/${ruta}/${filename}` : `${window.location.origin}/${ruta}/${filename}` ;
...
function buscar_sunat_reniec(formulario= '', input='', tipo_documento, dniruc, nombre, apellido, direccion, distrito, titular) {
  ...
  if (tipo_doc == "1") { // DNI
    ...
    if (dni_ruc.length == "8") {
      ...
      $.post("../ajax/ajax_general.php?op=reniec_jdl_paga", { dni: dni_ruc }, function (data, status) {
    ...
    if (dni_ruc.length == "11") {
      $.post("../ajax/ajax_general.php?op=sunat_jdl_paga", { ruc: dni_ruc }, function (data, status) {
        ...
        if (data == null || data.length === 0) {
          ...
          if (data.success == false) {
            ...
            if (data.estado == "ACTIVO") {
              ...
              var api_direccion       = (data.direccion_completa == null ? "" : data.direccion_completa);
...
function DocExist(url) {  
  ...
  var host = window.location.host == 'localhost' || es_numero(parseFloat(window.location.host)) == true ? `${window.location.origin}/libreria_sistema/${url}` : `${window.location.origin}/${url}`;
