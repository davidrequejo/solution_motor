<?php
  ...
  class Facturacion
  {
    ...
    public function listar_tabla_facturacion( $fecha_i, $fecha_f, $cliente, $tipo_persona, $comprobante, $metodo_pago, $centro_poblado, $estado_sunat ) {    
      ...
      WHERE vw_f.estado_v = 1 AND vw_f.estado_delete_v = 1 AND vw_f.tipo_comprobante in ('01', '03', '07', '12') $filtro_id_trabajador $filtro_id_punto $filtro_cliente $filtro_tipo_persona $filtro_comprobante $filtro_metodo_pago $filtro_centro_poblado $filtro_estado_sunat $filtro_fecha      
    ...
    public function insertar(
      ...
      //DATOS TABLA: venta cuota 
      $vc_idventa, $f_venta_cuotas, $fecha_cuota, $monto_cuota, $idventa_cuotas, $pagar_cuota
    ){  
      ...
      if ($f_venta_cuotas == 'NO' && (empty($vc_idventa) || $vc_idventa == 0 )  ) {
        $fecha_cuota = []; $monto_cuota = [];
      }
      ...
      if ($tipo_comprobante == '03' || $tipo_comprobante == '01' ) {
        ...
        if ( empty($val_cliente['data']) ) {
          ...
          if ( empty($val_cliente['data']['numero_documento']) ) {
            return array( 'status' => 'error_usuario', 'code_error' => '', 'titulo' => 'No tiene Numero Documento!!', 'message' => 'Este cliente: ' . $val_cliente['data']['cliente_nombre_completo'] . ' no cuenta con un Numero de Documento valido para generar BOLETA o FACTURA, porfavor actualice los datos' , 'user' =>  $_SESSION['user_nombre'], 'data' => [], 'id_tabla' => '' );          
          }
        }
      }
      ...
      impuesto, venta_subtotal, venta_descuento, venta_igv, venta_total, venta_cuotas, usar_anticipo, ua_monto_disponible, ua_monto_usado, nc_motivo_nota, 
      nc_tipo_comprobante, nc_serie_y_numero, cot_tiempo_entrega, cot_validez, cot_estado, observacion_documento) 
      ...
      if ( !empty($newdata['data']) ) {      
        while ($i < count($idproducto_presentacion)) {
          $subtotal_compra = $cantidad_total[$i] * $precio_compra[$i];
          ...
          precio_venta_descuento, descuento, descuento_porcentaje, subtotal, subtotal_compra, subtotal_no_descuento, um_nombre, um_abreviatura, precio_por_mayor)
          ...
          '$precio_venta_descuento[$i]', '$f_descuento[$i]', '$descuento_porcentaje[$i]', '$subtotal_producto[$i]', '$subtotal_compra', '$subtotal_no_descuento_producto[$i]', '$um_nombre[$i]', '$um_abreviatura[$i]', '$precio_por_mayor[$i]');";
          $detalle_new =  ejecutarConsulta_retornarID($sql_2, 'C'); if ($detalle_new['status'] == false) { return  $detalle_new;}          
          $id_d = $detalle_new['data'];            

          // Reducimos el Stock
          $sql_2_1 = "UPDATE producto_sucursal set  stock = stock - $cantidad_total[$i] 
          where idproducto_sucursal = (SELECT DISTINCT idproducto_sucursal FROM producto_presentacion where idproducto_presentacion ='$idproducto_presentacion[$i]') ;";
      ...
      $cant_cuotas = 0;
      if ( $f_venta_cuotas == 'SI' && (empty($vc_idventa) || $vc_idventa == 0 )  ) {
        foreach ($fecha_cuota as $key => $val) {     
          $cant_cuotas = ($key+1);     
          $sql_4 = "INSERT INTO venta_cuotas(idventa, numero_cuota, fecha_vencimiento,  monto_cuota)
          VALUES ('$id', '$cant_cuotas', '$fecha_cuota[$key]', '$monto_cuota[$key]');";
          $cuota_new =  ejecutarConsulta_retornarID($sql_4, 'C'); if ($cuota_new['status'] == false) { return  $cuota_new;}            
        }
      } else {

        if ( !empty($vc_idventa || $vc_idventa != 0 ) ) {
          foreach ($pagar_cuota as $key => $val) {
            if ( $val == 'on') {
    ...
    public function editar( // SI SE EDITA UNA VENTA, SE EDITA EN DIFERENTE CAJA??
      ...
      $f_descuento, $descuento_porcentaje, $subtotal_producto, $subtotal_no_descuento_producto, $precio_por_mayor,
      //DATOS TABLA: venta cuota 
      $f_venta_cuotas, $fecha_cuota, $monto_cuota
    ) {
      ...
      $val_venta = "SELECT * FROM venta WHERE idventa = '$idventa' AND venta_cuotas = 'SI' AND vc_estado in ('pagado', 'parcial')"; ejecutarConsulta($val_venta);
      if ( !empty($val_venta['data']) ) {
        if ( $val_venta['data']['venta_cuotas'] != $f_venta_cuotas ) {
          return $datos = [
            'status' => 'error_personalizado', 'message' => 'No puede quitar el tipo de pago credito porque ya tiene un pago', 'titulo' => 'No puedes desactivar!!' ,  'user' => $_SESSION['user_nombre'], 
            'data' => []
          ];
        }
      }
      ...
      foreach ($idproducto_presentacion as $key => $val) {
        ...
        $sql_2_1 = "UPDATE producto_sucursal set  stock = stock + IFNULL((select cantidad_total from venta_detalle where idproducto_presentacion = '$val' and idventa = '$idventa'), 0) 
      ...
      $cant_cuotas = 0;
      if ( !empty($fecha_cuota) ) {
        foreach ($fecha_cuota as $key => $val) {               
          $cant_cuotas = ($key+1);     

          $sql_4 = "SELECT * from venta_cuotas where idventa = '$idventa' and estado = '1' and estado_delete = '1' and numero_cuota = $cant_cuotas ";
          $existe_cuota =  ejecutarConsultaSimpleFila($sql_4); if ($existe_cuota['status'] == false) { return  $existe_cuota;} 
          
          if ( empty($existe_cuota['data'])) {
            $sql_4 = "INSERT INTO venta_cuotas(idventa, numero_cuota, fecha_vencimiento,  monto_cuota)
            VALUES ('$idventa', '$cant_cuotas', '$fecha_cuota[$key]', '$monto_cuota[$key]');";
            $cuota_new =  ejecutarConsulta_retornarID($sql_4, 'C'); if ($cuota_new['status'] == false) { return  $cuota_new;}    
          } else {
            $numero_cuota_edit = $existe_cuota['data']['numero_cuota'];
            $sql_4 = "UPDATE venta_cuotas SET fecha_vencimiento = '$fecha_cuota[$key]',  monto_cuota = '$monto_cuota[$key]'
            WHERE idventa = '$idventa' AND numero_cuota = '$numero_cuota_edit';";
            $cuota_new =  ejecutarConsulta_retornarID($sql_4, 'C'); if ($cuota_new['status'] == false) { return  $cuota_new;}    
          }                  
        }
      } else {       
    ...
    public function actualizar_respuesta_sunat( $idventa, $sunat_estado , $sunat_observacion, $sunat_code, $sunat_hash, $sunat_mensaje, $sunat_error, $sunat_ticket) {
      ...
      sunat_hash='$sunat_hash',sunat_mensaje='$sunat_mensaje', sunat_error = '$sunat_error', sunat_ticket='$sunat_ticket' WHERE idventa = '$idventa';";
    ...
    public function actualizar_doc_anulado_x_nota_credito( $idventa) {
      ...
      $sql_1 = "UPDATE venta SET sunat_estado='ANULADO' WHERE idventa = (
        SELECT * FROM (
    ...
    public function agregar_nuevo_cliente( $cli_tipo_persona_sunat, $cli_idtipo_persona, $cli_tipo_documento, $cli_numero_documento, $cli_nombre_razonsocial, $cli_apellidos_nombrecomercial, $cli_correo,$cli_celular,
      ...
      if ( empty($buscando['data']) || $cli_tipo_documento == '0' ) {
        ...
        foreach ($buscando['data'] as $key => $value) {
          $info_repetida .= '<li class="text-left font-size-13px">
            <span class="font-size-15px text-danger"><b>Nombre: </b>'.$value['persona_nombre_completo'].'</span><br>
