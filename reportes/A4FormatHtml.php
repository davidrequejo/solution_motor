...
if (!isset($_SESSION["user_nombre"])) {
  ...
  if ($_SESSION['facturacion'] == 1) { 
    ...
    $user_en_atencion     = empty($venta_f['data']['venta']['user_en_atencion']) ? '':mb_convert_encoding($venta_f['data']['venta']['user_en_atencion'], 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['user_en_atencion'], "UTF-8, ISO-8859-1, ISO-8859-15", true));
    ...
    $gr_modalidad_traslado= empty($venta_f['data']['venta']['gr_modalidad_traslado']) ? '':mb_convert_encoding($venta_f['data']['venta']['gr_modalidad_traslado'], 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['gr_modalidad_traslado'], "UTF-8, ISO-8859-1, ISO-8859-15", true));
    $gr_motivo_traslado   = empty($venta_f['data']['venta']['gr_motivo_traslado']) ? '':mb_convert_encoding($venta_f['data']['venta']['gr_motivo_traslado'], 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['gr_motivo_traslado'], "UTF-8, ISO-8859-1, ISO-8859-15", true));
    ...
    $gr_placa             = empty($venta_f['data']['venta']['gr_placa']) ? '':mb_convert_encoding($venta_f['data']['venta']['gr_placa'], 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['gr_placa'], "UTF-8, ISO-8859-1, ISO-8859-15", true));
    ...
    $observacion_documento= empty($venta_f['data']['venta']['observacion_documento']) ? '':mb_convert_encoding($venta_f['data']['venta']['observacion_documento'], 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['observacion_documento'], "UTF-8, ISO-8859-1, ISO-8859-15", true));
    $sunat_hash           = empty($venta_f['data']['venta']['sunat_hash']) ? '':mb_convert_encoding($venta_f['data']['venta']['sunat_hash'] ?? '', 'UTF-8', mb_detect_encoding($venta_f['data']['venta']['sunat_hash'] ?? '', "UTF-8, ISO-8859-1, ISO-8859-15", true) ?: 'UTF-8');

    $venta_cuotas         = $venta_f['data']['venta']['venta_cuotas'];
    $html_cuotas = '';
    foreach ($venta_f['data']['venta_cuotas'] as $key => $val) {
      $html_cuotas .= '<tr><td style="text-align: center;"> '.$val['numero_cuota'].' </td>   <td style="text-align: center;">'.$val['fecha_vencimiento_format'].'</td> <td style="text-align: right;"> '. number_format(floatval($val['monto_cuota']) , 2, '.', ',').'</td> </tr>';
    }
    ...
    foreach ($venta_f['data']['detalle'] as $key => $val) { 
      ...
      $html_venta .= '<tr class="item-list">'.       
        '<td style="text-align: center; padding-left: 8px; font-size: 10px;">' .  floatval($val['cantidad_total'])  . '</td>' .
        '<td style="text-align: center; padding-left: 8px; font-size: 10px;">' .  $val['um_nombre']  . '</td>' .
        '<td style="text-align: left;   padding: 0.5rem; font-size: 10px; word-break: break-all;">' . ($val['codigo']) . '</td>' .
        '<td style="text-align: left;   padding: 0.5rem; font-size: 10px; overflow-wrap: break-word;">' . ($val['pr_nombre']) . '</td>' .
        ($tipo_comprobante == '09' ? '' : 
    ...
    $dir = __DIR__ . '/generador-qr/ticket';
    ...
    // Etiqueta (API v6 usa constructor, no ::create)
    $label = new Label(
        text: $serie_y_numero_comprobante,
        textColor: new Color(255, 0, 0)
    );

    // Escribir PNG y guardar
    $writer = new PngWriter();
    $result = $writer->write($qrCode, null, $label);

    $result->saveToFile($dir . '/' . $filename);
    $logoQr = $result->getDataUri(); // para <img src="...">
    ...
    <div style="padding: 10px;"> 
      ...
      <div class="flex header-primary" style="margin-bottom: 15px;">
        ...
        <div style="width: 50%; padding-left: 0px; padding-right: 10px;">         
          <div class="image-logo-container"> 
            <center> <div class="image-logo" style="background-image: url(&quot;<?php echo $e_logo_empresa;?>&quot;);"></div></center>
          </div>
        </div>
        ...
        <table role="grid" class="table-products" style="table-layout: fixed;">
          <thead>
            <tr role="row">
              ...
              <?php if ($tipo_comprobante == '09'  ) {?>
                <!-- En guia no hay montos -->
              <?php } else { ?>
      ...
      <?php if( $tipo_comprobante == '09'){ ?>
        <div style="border: 1px solid gray; padding: 7px; font-size: 12px; border-radius: 7px; margin-top: 15px; ">        
          <table border='0' align="start" width='100%'  style='font-size: 12px' >   
            <tr>
              <th style="text-align: left;" colspan="2" > DATOS RELACIONADOS CON EL TRASLADO DE LOS BIENES</th>                
            </tr>   
            <tr >
              <td style="text-align: left; margin-top: 5px;" colspan="2"><b>Motivo traslado:</b> <br> <?php echo $gr_motivo_traslado;?> </td>  
            </tr>                   
            <tr > 
              <td style="text-align: left; margin-top: 5px; "> <b>Direccion punto de partida:</b>  <br> <?php echo $gr_partida_direccion;?> </td>  
              <td style="text-align: left; margin-top: 5px;"><b>Ubigeo punto de partida:</b> <br> <?php echo $gr_partida_ubigeo .' ' . $gr_partida_distrito;?></td>               
            </tr>  
            <tr >
              <td style="text-align: left; margin-top: 5px;"><b>Direccion punto de llegada:</b> <br> <?php echo $gr_llegada_direccion;?> </td>  
              <td style="text-align: left; margin-top: 5px;"><b>Ubigeo punto de llegada:</b> <br> <?php echo $gr_llegada_ubigeo . ' '.$gr_llegada_distrito  ;?></td> 
            </tr>    
        ...
        <div style="border: 1px solid gray; padding: 7px; font-size: 12px; border-radius: 7px; margin-top: 15px; ">        
          <table border='0' align="start" width='100%'  style='font-size: 12px' >   
            <tr>
              <th style="text-align: left;" colspan="3" > DATOS RELACIONADOS CON LA UNIDAD DE TRANSPORTE Y EL CONDUCTOR</th>                
            </tr>   
            <tr >
              <td style="text-align: left; margin-top: 5px;" colspan="3"><b>Modalidad Traslado:</b> <br> <?php echo $gr_modalidad_traslado;?> </td>  
            </tr>                   
            <tr > 
              <td style="text-align: left; margin-top: 5px; "> <b>Nombres y apellidos o razón social del transportista:</b>  <br> <?php echo $gr_chofer_nombre;?> </td>  
              <td style="text-align: left; margin-top: 5px;"><b>Tipo y Nro. Doc. Conductor:</b> <br> <?php echo $gr_chofer_nombre_tipo_documento .': ' . $gr_chofer_numero_documento;?> <br> Licencia: <?php echo $gr_numero_licencia ;?>  </td>               
              <td style="text-align: left; margin-top: 5px;"><b>Placa del vehículo:</b> <br> <?php echo $gr_placa ;?></td>               
            </tr>                      
          </table>         
        </div>
        <?php } ?>
      ...
      <div style="margin-top: 15px;">
        ...
        <div style="border: 1px solid gray; padding: 7px; font-size: 12px; border-radius: 7px; margin-top: 15px;">
          <div>
            ...
            <?php if ($tipo_comprobante == '01' || $tipo_comprobante == '03' || $tipo_comprobante == '12' ) {?> | 
        ...
        <?php if( $venta_cuotas  == 'SI'){ ?>
        <div style="border: 1px solid gray; padding: 7px; font-size: 12px; border-radius: 7px; margin-top: 15px; width: 350;">        
          <table border='0' align="start" width='300px'  style='font-size: 12px' >                     
            <tr><th > # Cuota</th>         <th>Fecha</th> <th> Monto </th> </tr>
            <?php echo $html_cuotas;?>
          </table>         
        </div>
        <?php } ?>
