...
function listar_tabla() {
  tabla = $('#tabla-gastos').dataTable({
    ...
    buttons: [
      ...
      { extend: 'excel', exportOptions: { columns: [0,8,9,10,11,2,12,13,4,14,15,5,16], }, title: 'Lista de gasto', text: `<i class="far fa-file-excel fa-lg" ></i>`, className: "btn btn-outline-success btn-wave ", footer: true, },
      { extend: 'pdf', exportOptions: { columns: [0,8,9,10,11,2,12,13,4,14,15,5,16], }, title: 'Lista de gasto', text: `<i class="far fa-file-pdf fa-lg"></i>`, className: "btn btn-outline-danger btn-wave ", footer: false, orientation: 'landscape', pageSize: 'LEGAL', },
...
function mostrar_editar_gdt(idgasto_de_trabajador) {
  ...
  $.post("../ajax/gasto_de_trabajador.php?op=mostrar_editar_gdt", { idgasto_de_trabajador: idgasto_de_trabajador }, function (e, status) {
    e = JSON.parse(e);
    if (e.status == true) {
      $("#idgasto_de_trabajador").val(e.data.idingreso_egreso_interno);
